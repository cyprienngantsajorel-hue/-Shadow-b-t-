  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    ...
    if (connection === "connecting" && !sock.authState.creds.registered) {
      ...
      try {
        ...
        console.log("\n╔══════════════════════════════╗");
        console.log("║     🔑 CODE DE JUMELAGE      ║");05112011
        console.log("╠══════════════════════════════╣");
        console.log(`║       ${fmt}        ║`);
        console.log("╚══════════════════════════════╝");
    ...
    setTimeout(async () => {
      await sock.sendMessage(data.from, {
        text:
          ...
          `⚡ *shadow-bot* est de retour !\n` +
          `📦 *Commandes :* ${plugins.size} chargées\n` +
          `🚀 *Statut :* Opérationnel !\n\n` +
          `_Powered by shadow-bot_ 🌸`,
      });
    }, 3000);
  } catch {}
}
...
if (isFromMe && !msg.key.participant) {
  ...
  return; // ✅ Ignore en groupe si c'est le bot
    ...
    const isCreator = senderNumber === creatorNum;242065699529
